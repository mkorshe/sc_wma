...
colnames_for_eqtl_polished<-  read.table('/groups/umcg-bios/tmp01/projects/1M_cells_scRNAseq/ongoing/LIMIX/WMA_round_II/pseudobulk/output/WMA/colnames_pairwise.tsv', sep='\t', header=T) 
...
dir.create(paste(output_path_for_summaries,  '5ds','/group_', N, sep=''), showWarnings = TRUE, recursive = FALSE, mode = "0777")
...
for(f in sequence_of_values){
  ...
  for(i in 1:nrow(eqtl)){
    ...
    weighted_z_score_se <- get_weighted_z_score_SE(zScores= as.numeric(c(eqtl[i,'corrected_zscoreng'], eqtl[i,'corrected_zscorestemi_v2'],eqtl[i,'corrected_zscorestemi_v3'],eqtl[i,'corrected_zscorev2_1m'],eqtl[i,'corrected_zscorev3_1m'])), weights=c(1,1,1,1,1),SEs=as.numeric(c(eqtl[i,'beta_seng'], eqtl[i,'beta_sestemi_v2'],eqtl[i,'beta_sestemi_v3'],eqtl[i,'betav2_1m'],eqtl[i,'betav3_1m'])))
    
    eqtl[i,"ZW_weight_SE"] <- (weighted_z_score_se)
  }
  eqtl[,'pvalue_ZW_SE'] <- convert.z.score(as.numeric(eqtl[,'ZW_weight_SE']))
  ...
  for (id in weighting_ids){
   ...
   # id = substring(id,1, nchar(id)-1)
    ...
    weight1 <- paste(id,"ng",sep='')
    weight2 <- paste(id,"stemi_v2",sep='')
    weight3 <- paste(id,"stemi_v3",sep='')
    weight4 <- paste(id,"v2_1m",sep='')
    weight5 <- paste(id,"v3_1m",sep='')
    for(i in 1:nrow(eqtl)){
      
      weighted_z_score_se <- get_weighted_z_score_SE(zScores= as.numeric(c(eqtl[i,'corrected_zscoreng'], eqtl[i,'corrected_zscorestemi_v2'],eqtl[i,'corrected_zscorestemi_v3'],eqtl[i,'corrected_zscorev2_1m'],eqtl[i,'corrected_zscorev3_1m'])),
                                                     weights=as.numeric(c(eqtl[i,weight1][[1]],eqtl[i,weight2][[1]],eqtl[i,weight3][[1]],eqtl[i,weight4][[1]],eqtl[i,weight5][[1]])),
                                                     SEs=as.numeric(c(eqtl[i,'beta_seng'], eqtl[i,'beta_sestemi_v2'],eqtl[i,'beta_sestemi_v3'],eqtl[i,'betav2_1m'],eqtl[i,'betav3_1m'])))
      
      eqtl[i,paste0("ZW_SE", id)] <- weighted_z_score_se
    }
    eqtl[,paste0("pvalue_SE", id)] <- convert.z.score(as.numeric(eqtl[,paste0("ZW_SE", id)]))
    
  }
  
  
  for(samplesize_id in samplesize_ids){
    weights_SS_counts <-  weights_ss[,samplesize_id]
    
    SS1=weights_SS_counts[3]
    SS2=weights_SS_counts[4]
    SS3=weights_SS_counts[5]
    SS4=weights_SS_counts[1]
    SS5=weights_SS_counts[2]
    
    sampleSizes =c(SS1,SS2,SS3, SS4, SS5)
    sampleSizes
    for(i in 1:nrow(eqtl)){
      
      weighted_z_score_ss <- get_weighted_z_score(zScores= as.numeric(c(eqtl[i,'corrected_zscoreng'], eqtl[i,'corrected_zscorestemi_v2'],eqtl[i,'corrected_zscorestemi_v3'],eqtl[i,'corrected_zscorev2_1m'],eqtl[i,'corrected_zscorev3_1m'])),sampleSizes = sampleSizes,  weights=c(1,1,1,1,1))
   ...
   #   id = substring(id,1, nchar(id)-1)
      ...
      for(i in 1:nrow(eqtl)){
        ...
        weighted_z_score_ss <- get_weighted_z_score(zScores= as.numeric(c(eqtl[i,'corrected_zscoreng'], eqtl[i,'corrected_zscorestemi_v2'], eqtl[i,'corrected_zscorestemi_v3'], eqtl[i,'corrected_zscorev2_1m'],eqtl[i,'corrected_zscorev3_1m'])), sampleSizes = sampleSizes, weights=c(as.numeric(eqtl[i,weight1][[1]]),as.numeric(eqtl[i,weight2][[1]]),as.numeric(eqtl[i,weight3][[1]]),as.numeric(eqtl[i,weight4][[1]]),as.numeric(eqtl[i,weight5][[1]])))
  ...
  param_cols1 <- colnames(eqtl)[grep('ng', colnames(eqtl))]
  param_cols2 <- colnames(eqtl)[grep('v2_1m', colnames(eqtl))]
  param_cols3 <- colnames(eqtl)[grep('v3_1m', colnames(eqtl))]
  param_cols4 <- colnames(eqtl)[grep('stemi_v2', colnames(eqtl))]
  param_cols5 <- colnames(eqtl)[grep('stemi_v3', colnames(eqtl))]
  
  eqtl_polished <- eqtl %>%  select(colnames_for_eqtl_polished[,'x'])
  
  # writing output
   # write.table(colnames(eqtl_polished), paste(output_path_for_summaries, "colnames.tsv", sep=''), sep = '\t', col.names = T, row.names = F, quote=F)
  
  write.table(eqtl_polished, paste(output_path_for_summaries, '5ds','/group_', N ,'/',f, ".tsv", sep=''), sep = '\t', col.names = F, row.names = F)
